// Se agregaron comentarios, se acomodaron las cosas con respecto al registro.txt, se corrigio el error de listar operaciones, ya funciona, se agrego la operacion de depositar.
...
#include <string.h>
#include <stdlib.h>
#define TAM1 10  // Constantes declaradas para ser utilizadas en el programa
#define TAM2 1000
    ...
    while(opcion!=6){
        ...
        do{
            ...
            switch(opcion){
                case 1:
                    //
                    cargar_operacion(binario); //Leo archivo de texto, guardo en registro los datos, le paso por parametro el binario
                ...
                case 4:
                    //
                    mostrar_informacion(binario); // Muestro toda la informacion de todas las cuentas almacenadas
                    break;

                case 5:
                    top3_maximosaldo(binario);  // Muestro el top 3 de los saldos maximos
                    break;
            }
    }
...
void listar_operaciones() // Con el formato de limitadores, uso el mismo metodo de lectura para comparar el numero de cuenta
{
    ...
    int contador= 0;
    struct registro DATO[TAM2]; // Vector de struct para guardar los numeros de cuenta en registro
...
void modificar_nombre()
{
   ...
   int validacion;
    ...
    if (binario==NULL){
        ...
        while(!feof(binario)){ // Bucle hasta llegar al final del archivo
            ...
            if (strcmp(numerodecuenta,nuevo_nombre.numero_cuenta)==0){                   // Comparo el numero de cuenta ingresada con los existentes
               ...
               validacion= ValidarNombre(nuevo_nombre.name);
               }while(validacion == 0);
